...
const Collapse = ({children, list=[], onClick, icon}) => {
  ...
  <div className="w-full">
    <button onClick={toggleCollapse} className="border text-[#696E76] dark:text-neutral-300 hover:text-primary dark:hover:text-primary hover:bg-slate-50 dark:hover:bg-neutral-950 flex justify-between items-center gap-x-4 w-full px-4 py-2 rounded-lg"><span className="flex gap-x-4 items-center">{icon} {children}</span> {isOpen ? <IoIosArrowUp /> : <IoIosArrowDown />}</button>
    <AnimatePresence>
      {
        isOpen && (
          <motion.div  
